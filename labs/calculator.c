...
modulo(int num,char op, int num_2); /* functions to do arithmitic */
...
int main()
{
	...
	while(input = getchar())
	{
		...
		{
			printf("no negatives"); /*checking for negatives */
	...
	while(input = getchar() )
	{
		...
		switch(input)		/* switch statement to determine operator sign */
		{
			case 37:
				op = 37;
				break;
	...
	while(input = getchar() )
	{
		...
		else if(op == 47 && input == 48) /* checks to not divide by zero */
		{
...
multiplication(int num,char op, int num_2) /* declaration functions for arithmetic */
{
	return num * num_2;
}
